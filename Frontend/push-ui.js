...
(function () {
  ...
  async function toggleNotifications() {
    ...
    if (Notification.permission === "denied") {
      showToast(
        "Notificaciones bloqueadas. Haz clic en el candado 🔒 de la URL para permitirlas.",
        "error"
      );
      return;
    }
  ...
  async function updateNotificationButtonState() {
    ...
    if (Notification.permission === "denied") {
      ...
      if (button)
        button.title =
          "Notificaciones bloqueadas - Haz clic para ver cómo habilitarlas";
      return;
    }
  ...
  function showToast(message, type = "info") {
    ...
    } text-white px-4 py-2 rounded-lg shadow-lg transform transition-all duration-300 translate-x-full`;
