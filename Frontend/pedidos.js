...
// ✅ --- CAMBIO CLAVE: Se ajusta la función para manejar fechas como UTC --- ✅
function formatDate(fecha) {
  if (!fecha) return 'Sin fecha';
  try {
      // Al recibir 'YYYY-MM-DD', new Date() lo interpreta como UTC a la medianoche.
      // Pero para ser 100% seguros, lo separamos y creamos la fecha en UTC.
      const parts = fecha.split('T')[0].split('-');
      if (parts.length !== 3) return fecha;

      const year = parseInt(parts[0], 10);
      const month = parseInt(parts[1], 10) - 1; // Meses en JS son de 0 a 11
      ...
      return date.toLocaleDateString('es-ES', {
          ...
          month: '2-digit',
          year: 'numeric',
          timeZone: 'UTC' // Le decimos que formatee la fecha como si estuviera en UTC
      });
  } catch {
      return fecha;
  }
...
function renderPedido(pedido) {
  console.log("Renderizando pedido con estado:", pedido.estado);
...
document.body.addEventListener("htmx:afterSwap", (event) => {
  ...
  try {
      ...
      // La respuesta de Go ahora es un objeto, no un array directamente.
      const data = JSON.parse(responseText); 
      const pedidos = data.pedidos || []; // Extraemos el array de pedidos
